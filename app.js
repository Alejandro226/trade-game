...
function renderHeader(){
  const u=state.user||{};
  $("#greet").textContent=u.username?u.username:"";
  $("#balance").textContent=u.balance!=null?("$"+fmt(u.balance)):"";
  $("#onlineCount").textContent=state.onlineSet.size?("Online "+state.onlineSet.size):"";
}
...
async function decideAuthUI(){
  ...
  if(data.username){ showApp(); } else { showAuth(); }
}
